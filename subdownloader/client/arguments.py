...
def get_argument_parser():
    ...
    interfaceGroup = parser.add_argument_group('interface', 'Change settings of the interface')
    guicli = interfaceGroup.add_mutually_exclusive_group()
    guicli.add_argument('-g', '--gui', dest='mode',
                        action='store_const', const='gui',
                        help='Run application in GUI mode. This is the default.')
    ...
    interfaceGroup.add_argument('-T', '--test', dest='test',
                                ...
                                help='Used by developers for testing. (unsupported)')
    interfaceGroup.add_argument('-V', '--video', dest='videofile',
                                metavar='PATH', default=None,
                                help='Full path to your video(s). Don\'t use "~".')  # FIXME: allow ~
    interfaceGroup.add_argument('-l', '--lang', dest='language', default='all',
                                help='Set the language of the subtitle for download and upload.')

    # logger options
    loggroup = parser.add_argument_group('logging', 'Change the amount of logging done.')
    loglvlex = loggroup.add_mutually_exclusive_group()
    loglvlex.add_argument('--debug', dest='loglevel',
                          action='store_const', const=logging.DEBUG,
                          help='Print log messages of debug severity and higher to stderr.')
    loglvlex.add_argument('--warning', dest='loglevel',
                          action='store_const', const=logging.WARNING,
                          help='Print log messages of warning severity and higher to stderr. This is the default.')
    loglvlex.add_argument('--error', dest='loglevel',
                          action='store_const', const=logging.ERROR,
                          help='Print log messages of error severity and higher to stderr.')
    loglvlex.add_argument('--quiet', dest='loglevel',
                          action='store_const', const=LOGGING_LOGNOTHING,
                          help='Don\'t log anything to stderr or stdout')
    loggroup.set_defaults(loglevel=logging.WARNING)

    loggroup.add_argument('--log', dest='logfile', metavar='FILE',
                          default=None, help='Path name of the log file.')

    # cli options
    cliGroup = parser.add_argument_group('cli', 'Change the behavior of the command line interface.')
    ...
    operationGroup.add_argument('-D', '--download', dest='operation', action='store_const', const='download',
                                help='Download a subtitle. This is the default.')
    ...
    renameGroup.add_argument('--keep-names', dest='renaming', action='store_false',
                             help='Keep original subtitle names. This is the default.')
    renameGroup.add_argument('--rename-subs', dest='renaming', action='store_true',
                             help='Rename subtitles to match the movie file name.')
